...
export const metadata: Metadata = {
  title: "Universities",
  description: "An empty layout scaffold for the Universities section.",
};

export default async function UniversitiesPage() {
  const locale = await getServerLocale();
  const breadcrumb = locale === "ro" ? "Universități" : "Universities";
    ...
    <main className="min-h-screen bg-navy-dark pt-24 text-white">
      ...
      >
        <div className="mx-auto max-w-4xl rounded-3xl border border-dashed border-white/15 bg-white/3 p-8">
          {/* TODO: Drop in existing thesis statement on American higher education excellence here */}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            <div className="min-h-[180px] rounded-2xl border border-dashed border-white/15 bg-white/3 p-6">
              {/* TODO: Create empty cards. Drop in links to Ivy League, STEM Powerhouses, Business Schools, Public Research here */}
            </div>
            <div className="min-h-[180px] rounded-2xl border border-dashed border-white/15 bg-white/3 p-6" />
            <div className="min-h-[180px] rounded-2xl border border-dashed border-white/15 bg-white/3 p-6" />
            <div className="min-h-[180px] rounded-2xl border border-dashed border-white/15 bg-white/3 p-6" />
