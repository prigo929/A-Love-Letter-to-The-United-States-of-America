...
import type { HouseholdNetWorthPoint } from "@/lib/data/economy-data";

interface HouseholdNetWorthChartProps {
  data: HouseholdNetWorthPoint[];
  title?: string;
  subtitle?: string;
  source?: string;
}

export function HouseholdNetWorthChart({ data, title, subtitle, source }: HouseholdNetWorthChartProps) {
  const { locale } = useLanguage();
  const ro = locale === "ro";

  const copy = ro
     ? {
         valueLabel: "Averea Netă Totatală a Gospodăriilor (Trilioane USD)",
         source: "Sursă:",
         latest: "Cel mai recent",
         growth: "Creștere din 1970",
       }
     : {
         valueLabel: "Total Household Net Worth (USD Trillions)",
         source: "Source:",
         latest: "Latest",
  ...
  const first = data[0];
  const growthMultiple = (latest.value / first.value).toFixed(1);
  ...
  return (
    ...
    >
            ...
            <AreaChart data={data} margin={{ top: 20, right: 20, left: 10, bottom: 5 }}>
              ...
              <Tooltip
                content={({ active, payload, label }) => {
                  ...
                  const p = payload[0].payload as HouseholdNetWorthPoint;
                  return (
                    <div className={CHART_TOOLTIP_CLASS}>
                      <p className="mb-1 font-body text-xs text-white/50">{String(label).slice(0, 7)}</p>
                      <p className="font-hero text-2xl text-glory-gold">${p.value.toFixed(2)}T</p>
      ...
      <div className="mt-8 grid grid-cols-2 gap-6 border-t border-white/10 pt-6">
        <div>
          <div className="font-hero text-3xl text-glory-gold md:text-4xl">${latest.value.toFixed(1)}T</div>
          <div className="mt-1 font-body text-xs uppercase tracking-wider text-white/40">
            {copy.latest} ({String(latest.date).slice(0, 7)})
          </div>
        </div>
        <div>
          <div className="font-hero text-3xl text-white/85 md:text-4xl">{growthMultiple}x</div>
          <div className="mt-1 font-body text-xs uppercase tracking-wider text-white/40">
            {copy.growth}
          </div>
        </div>
