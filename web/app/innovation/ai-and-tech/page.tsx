      ...
      <div className="bg-[#000000] relative z-10 pb-32 pt-16 font-body text-white">
        ...
        <section id="software-moat" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-32 mb-32">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-0 border border-white/5 rounded-3xl overflow-hidden">
            {[
              { value: "92%",   label: isRo ? "Cotă de piață AI compute" : "AI compute market share", note: isRo ? "Nvidia CUDA vs. toți rivalii" : "Nvidia CUDA vs. all rivals" },
              { value: "~20yr", label: isRo ? "Avans față de concurență" : "Head start on competition", note: isRo ? "Lansat 2006, neegalat încă" : "Launched 2006, unmatched today" },
              { value: "4M+",   label: isRo ? "Developeri în ecosistem" : "Developers in ecosystem", note: isRo ? "Imposibil de migrat rapid" : "Impossible to migrate quickly" },
              { value: "500K+", label: isRo ? "Modele optimizate CUDA" : "CUDA-optimized models", note: isRo ? "Pe HuggingFace Hub" : "On HuggingFace Hub" },
            ].map((s, i) => (
          ...
          <p className="text-xs font-body text-white/35 mt-4">
            ...
            {copy.labsList.map((lab, idx) => (
              ...
              >
                <div>
                  ...
                  <span className="text-[10px] font-body font-semibold text-[#E8B923]/70 uppercase tracking-widest block mb-4">
                    {lab.role}
                  </span>
